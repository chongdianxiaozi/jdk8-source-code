...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
        ...
        //node 是共享模式(同步队列单独的属性)
        static final Node SHARED = new Node();
        /** Marker to indicate a node is waiting in exclusive mode */
        //node 是排它模式(同步队列单独的属性)
     ...
     */
    // 线程加入同步队列中方法，追加到队尾
    // 这里需要重点注意的是，返回值是添加的 node 的前一个节点
    ...
    private void unparkSuccessor(Node node) {
        ...
        if (s == null || s.waitStatus > 0) {
            ...
            // 主要是因为节点被阻塞的时候，是在 acquireQueued 方法里面被阻塞的，
            // 唤醒时也一定会在 acquireQueued 方法里面被唤醒，唤醒之后的条件是，
            // 判断当前节点的前置节点是否是头节点，这里是判断当前节点的前置节点，
            // 所以这里必须使用从尾到头的迭代顺序才行，目的就是为了过滤掉无效的前置节点，
    ...
    // 2：获得锁的线程执行完成之后，释放锁时，会把阻塞的 node 唤醒,
    // node 唤醒之后再次自旋，尝试获得锁
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private void doSignal(Node first) {
            do {
                ...
                if ( (firstWaiter = first.nextWaiter) == null)
                    lastWaiter = null;
                // 从队列头部开始唤醒，所以直接把头节点.next 置为 null，
                // 这种操作其实就是把 node 从条件队列中移除了
             ...
             // 等待队列的 node 不用管，在 await 的时候，会自动清除状态不是 Condition 的节点(通过 unlinkCancelledWaiters 方法)
        ...
        private void unlinkCancelledWaiters() {
            Node t = firstWaiter;
            // trail 表示上一个状态,这个字段作用非常大，
            // 可以把状态都是 CONDITION 的 node 串联起来，即使 node 之间有其他节点都可以
            Node trail = null;
            while (t != null) {
